...
then
    ...
    coaddDriver.py ../data --rerun coadd:coaddPhot $Z_VISITS --id filter=VISTA-Z tract=$tract patch=$patch --reuse-outputs-from all --clobber-config --no-versions --cores=$numCPUs
fi
...
then
    ...
    coaddDriver.py ../data --rerun coadd:coaddPhot $Y_VISITS --id filter=VISTA-Y tract=$tract patch=$patch --reuse-outputs-from all --clobber-config --no-versions --cores=$numCPUs
fi
