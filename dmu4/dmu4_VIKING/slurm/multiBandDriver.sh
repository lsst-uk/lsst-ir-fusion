...
then
    ...
    multiBandDriver.py $repo --rerun coadd --id filter=VISTA-Z^VISTA-Y^VISTA-J^VISTA-H^VISTA-Ks^HSC-G^HSC-R^HSC-I^HSC-Z^HSC-Y tract=$tract patch=$patch --reuse-outputs-from all --no-versions --cores=5
fi
